#ifndef SANCTIFY_GAME_CLIENT_SRC_UTIL_RESOURCE_REGISTRY_H
#define SANCTIFY_GAME_CLIENT_SRC_UTIL_RESOURCE_REGISTRY_H
 ...
 * modified/deleted over the course of using the registry. Useful for
 * large resources that should not be copied around often.
 *
 * This object has NO synchronization! Perform all mutations on the
 * main thread, and only during times where no reads are being performed
 * on side threads!
 *
 * Also - notice this does NOT clean up memory, please destruct the entire
 * object when the resource registry may be bloated (e.g., between scenes)
 * and only use the registry for long-lived objects
 *
 * Intended for use in ECS systems where heavy CPU and/or GPU objects
 * are used - use should be short-lived (e.g. no persistant references).
 */
template <typename T>
